...
public class EventTrackController(ITrackService trackService) : ControllerBase
{
    ...
    [HttpGet]
    [AllowAnonymous]
    public async Task<IActionResult> GetTracks(Guid eventId, [FromQuery] BaseQueryParams request)
    {
        var result = await trackService.GetTracksByEventIdAsync(eventId, request);
